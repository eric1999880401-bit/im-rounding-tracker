...
export function makeRoundSoapPrompt(params: RoundSoapPromptParams) {
  return [
    ...
    "- HARD SIZE LIMITS: headerLines <=2, subjectiveLines <=4, vitalSigns <=1, physicalExam <=3, labs <=6 panel objects, microbiology <=4, imaging <=4 studies, pathology <=3, assessmentPlan <=6 problems, orders <=6, tasks <=6, discharge <=4, warnings <=4, highlightHints <=8.",
    "- Keep each string to one concise clinical sentence. Empty arrays are valid when the source has no supported content; never fill them with generic prose.",
    "- headerLines contain a concise patient identifier/one-liner plus active admission diagnosis, and one compact safety line when explicitly supplied: Code, Allergy, Isolation, HD/POD. Never guess a missing safety value. Never copy PMH, HPI, Lab, Image, consult text, or the admission transcript into the header.",
    "- subjectiveLines contain concise display-ready interval symptoms/events without bullets or section prefixes.",
    ...
    "- Put the current T/BP/HR/RR/SpO2 and O2 device in one vitalSigns line. Never emit SpO2 or oxygen support as a separate second V/S line.",
    "- objective.labs contains compact panel selections. Use CBC/DC, Chem/Renal, Liver/Coag, Infx/Perfusion, Urinalysis, ABG/VBG, Fluid studies, Cardiac, or Other. Never emit one object per analyte.",
    "- Each lab object must include sourceIds selected from patientContext.labFacts. First assess the whole patient and active problems, define the smallest monitoring set for each active lab-driven problem, then select the facts that establish severity, trajectory, treatment response, toxicity, or a pending diagnostic question. Do not select a fixed universal panel.",
    "- sourceIds and their associated specimen, analyte, value, unit, and prior result are application-owned source facts. Specimen identity is inseparable from the analyte and value. You may prioritize and summarize those facts, but never invent, recalculate, relabel, reinterpret, or rewrite them. The values field is explanatory only and must agree with those IDs.",
    "- Put joint/synovial fluid, CSF, pleural fluid, ascitic/peritoneal fluid, pericardial fluid, and BAL results under Fluid studies. Never reinterpret their WBC, RBC, glucose, protein, LDH, or other analytes as peripheral blood, serum, CBC/DC, or Chem/Renal results.",
    "- Never infer systemic leukocytosis, leukopenia, anemia, polycythemia, hypoglycemia, hyperglycemia, or serum protein/LDH abnormalities from a body-fluid value. Interpret a fluid result only in its explicitly supplied specimen context.",
    ...
    "- Do not leave objective.labs empty when supplied current lab facts establish severity, trajectory, treatment response, toxicity, or an unresolved question for an active problem.",
    "- Coverage check, not a universal checklist: infection/sepsis usually needs supplied WBC plus relevant CRP/PCT/lactate/ESR and cultures; neutropenic fever needs supplied WBC/ANC and relevant cultures; UTI/pyuria needs supplied Urinalysis and urine culture; AKI/lyte problems need Cr/eGFR/Na/K; liver injury needs AST/ALT/T-bil/INR; DKA/HHS needs supplied glucose/K/Na/HCO3/pH/ketone/osmolality; ACS needs troponin (and CK-MB if supplied); HF may need BNP/NT-proBNP plus renal/lytes; respiratory failure may need pH/pCO2/pO2/HCO3; bleeding/DIC needs Hb/Plt/coagulation/fibrinogen/D-dimer; pancreatitis needs lipase; TLS/rhabdomyolysis needs supplied K/P/Ca/Cr/uric acid/LDH/CK. Select only facts actually supplied and relevant to this patient.",
    "- Keep CBC/DC as one panel selection and preserve supplied WBC, Hb, and Plt together for orientation; add Neu/differential when clinically useful. Keep BUN/Cr/eGFR/electrolytes on Chem/Renal. CRP/PCT/lactate/ESR belong to Infx/Perfusion. UA WBC/RBC/LE/nitrite/bacteria belong to Urinalysis and must never be relabeled as blood CBC.",
    ...
    "- For every selected disease-relevant analyte, compare the current result with the immediately prior supplied result for the same analyte in the same specimen from patientContext.labFacts or the reviewed baseline when available. Preserve that prior value compactly in parentheses, e.g. 'Cr 2.1(2.7) down'. Never compare across specimens or with a non-adjacent historical value, and do not invent reference ranges or abnormal flags.",
    ...
    "- Consult assessments/recommendations are not admission text and are not generic O lines. Put a consult's supported clinical conclusion under the owning A/P problem and its concrete recommendation under that problem or Tasks.",
    "- Each imaging object must cite sourceIds from patientContext.imageFacts. Comparison statements, technique/protocol text, study titles without findings, and phrases ending in 'showed:' are context only and must never become imaging objects.",
    "- Synthesize the selected image evidence into one short actionable finding per study. Preserve supplied measurements/dates exactly; never add a number not present in the cited image facts.",
    "- assessmentPlan contains active problem blocks only. Each block has one clinically meaningful problemTitle, an explicit status (worsening, active, uncertain, improving, or stable), one synthesis summary, one concrete source-supported plan, and exact sourceEvidence snippets.",
    "- Order assessmentPlan deterministically by current clinical priority: worsening/unstable threats first, then active unresolved problems, then improving/stable problems. Do not let source paragraph order determine clinical priority.",
    ...
    "- warnings list actionable source conflicts or missing high-safety evidence (for example Code/Allergy, current V/S or Cr/K/Hb when clinically required, antibiotic culture/day count/duration, or a missing plan). highlightHints contains short exact source-grounded phrases worth visual emphasis.",
    "",
    ...
    "- Every infection/sepsis A/P must identify the supported anatomic/clinical source (for example PNA, UTI, cholangitis, CNS, wound, line/port). If the source is not established, write 'source unclear'; never output only 'infection + Abx'.",
    ...
    "- Lab trajectory: every selected disease-relevant current lab includes the immediately prior supplied value when available; no old value is presented as current, and no relevant current lab is silently omitted.",
    ...
    "Canonical image evidence is listed in patientContext.imageFacts as [source ID]; study/date; evidence. Cite only those IDs; the application rejects raw report boilerplate and unsupported numbers.",

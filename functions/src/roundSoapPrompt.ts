...
export function makeRoundSoapPrompt(params: RoundSoapPromptParams) {
  return [
    ...
    "- O order: V/S, PE, Lab, Image, then Pathology/other objective findings. Imaging never belongs in PE.",
    ...
    "- A final pathology/biopsy result must appear once in O as 'Pathology:' with specimen/study, date, and exact source wording. 's/p biopsy' is only a procedure status and must not be presented as the final pathology result.",
    ...
    "- Active antimicrobials are a deliberate cross-section exception: summarize the exact current regimen once under the matching infection A/P, and repeat the concise authoritative regimen as an Order line when order details are supplied.",

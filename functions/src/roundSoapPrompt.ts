...
export function makeRoundSoapPrompt(params: RoundSoapPromptParams) {
  return [
    ...
    "- headerLines contain only a concise patient identifier/one-liner and active admission diagnosis. Never copy PMH, HPI, Lab, Image, consult text, or the admission transcript into the header.",
    ...
    "- Each lab object must include sourceIds selected from patientContext.labFacts. First assess the whole patient and active problems, then select the facts that establish severity, trajectory, treatment response, toxicity, or a pending diagnostic question. Do not select a fixed universal panel.",

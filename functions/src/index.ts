...
function makeBatchImportPrompt(
  ...
  return [
    ...
    "- admissionSummary: write the same 1-min oral brief style: why admitted, focused HPI/ED or transfer course, key PMH/context, objective anchors, active assessment, today/pending/dispo; leave empty if there is no admission/course context.",
...
function makePrompt(sourceType: SourceType, rawText: string, patientContext: Record<string, unknown> | undefined) {
  ...
  const intakeTargets = [
    ...
    ...admissionSummaryStyleBullets.map((line) => `- ${line}`),
    "- admissionSummary: write the same 1-min oral brief style: admission reason, focused HPI/ED or transfer course, key PMH/context, objective anchors, active assessment, today/pending/dispo. Leave empty only if the pasted text has no admission/course context.",

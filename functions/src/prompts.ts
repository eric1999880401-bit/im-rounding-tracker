...
export const admissionSummaryStyleBullets = [
  "Admission brief format: a structured English case brief with labeled sections, each label starting its own line, in exactly this order: optional bed line (e.g. 'C15-03') when known from context; age+sex line (e.g. '90F'); 'PHx:'; 'CC:'; 'PI:'; 'ED Lab:' (or 'Lab [date]:' for non-ED data); 'Image:'; 'ED Course:' when ED events/resuscitation/empiric treatment exist; 'Imp:'; then 'Plan:' only when a concrete initial plan is in the source.",
  "English only with standard clinician Latin shorthand allowed (c for with, s/p, r/o, f/u). No Chinese connectives, no markdown asterisks, no headings other than the section labels above.",
  ...
  "You may wrap the 3-6 most decision-relevant terms (working dx, key abnormal values, key drugs) in the app's highlight markup like [[yellow:cellulitis]]; never use markdown ** bold.",

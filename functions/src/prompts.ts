...
export function makeRoundSoapPrompt(params: {
  ...
  return [
    ...
    "- A/P must use '# problem' blocks. Use only the number of active clinical problems supported by the source; do not target a fixed count.",
    ...
    "- Do not re-list the raw symptom enumeration that already appears in S under an A/P problem. The A/P assessment is a judgment (severity, trajectory, working dx), not a copy of the subjective complaints. Bad: '# Cellulitis/sepsis - Fever, vomiting, hypotension, lactate elevation, most c/w soft tissue infection'. Good: '# Lt thigh cellulitis c sepsis/SIRS - lactate 20.6->; on curam + clinda'.",
    "- Do not end an A/P line by restating the problem title in other words (no 'most c/w [same diagnosis]', 'consistent with [same dx]' when the title already says it). The diagnosis is the heading; the bullet is status + plan only.",
    "- Prefer '+' over the word 'and' to join co-listed drugs/findings (e.g. 'curam + clinda', 'fever + chills').",
    ...
    "- Tasks contain only actionable, timed, or pending one-time items. Do not create filler to reach a target count.",
    ...
    "- Header Dx items are short problem labels without evidence clauses: 'suspected UTI', not 'suspected UTI/pyuria c fever' — the pyuria and fever belong in S/O.",
    "- SpO2 always carries the oxygen device from the source ('SpO2 95% w/ N/C 2L'). Never write RA/room air unless the source explicitly says room air.",
    "- No editorial O lines like 'hemodynamically stable' when the V/S numbers are already shown; the numbers speak. Include a PE line only when it informs an active problem.",
    "- State an imaging finding once: 'B/L pleural effusions', not 'B/L pleural effusions/blunted CP angle' (same finding twice). Do not list known catheters/lines (Hickman, port, PICC) as image findings when they already appear under an A/P problem.",
    "- One data bullet per A/P problem: never add a second bullet that restates the same lab value with only vague follow-up ('- Hb 7.3; f/u trend' next to '- Hb 7.6 -> 7.3, stool neg; ...'). Merge into the single concrete bullet (trend + context + decision).",
    "- Never end a line with decision-deferral filler: 'as ordered', 'per team decision', 'adjust accordingly'. Either state the parameter or stop at the action.",

...
export function makeBatchImportPrompt(
  ...
  return [
    ...
    "- admissionSummary: use the structured admission brief format defined above (age/sex line, PHx:, CC:, PI:, PE:, Lab:, Image:, Imp:), preserving the line breaks between sections; leave empty if there is no admission/course context.",
...
export const admissionSummaryStyleBullets = [
  "Admission brief format: a structured English case brief with labeled sections, each label starting its own line, in exactly this order: line 1 age+sex only (e.g. '73F'); 'PHx:'; 'CC:'; 'PI:'; 'PE:' (write 'ED PE:' when the exam is from the ED); 'Lab [date]:'; 'Image:'; 'Imp:'; then 'Plan:' only when initial plan/pending items are clearly known from the source.",
  "English only. No Chinese connectives, no markdown bold/asterisks, no headings other than the section labels above.",
  "PHx: one comma-separated line of comorbidities in abbreviations (dementia, HTN, recent H. pylori infx s/p EGD). Never list both a full name and its abbreviation.",
  "CC: chief complaint compressed with duration, e.g. 'painful dermatomal rash x recent days'.",
  "PI: 3-6 short chronological lines, each one compressed clinical sentence. Use '\u2192' for progression/response, 'x [duration]' for time course, and slash-joined pertinent negatives. Include prior workup with its result.",
  "PE: focused positive findings only, 1-2 lines, with laterality/location abbreviations (bil, Lt/Rt, RLE, T5 dermatome).",
  "Lab [date]: compact panels without units, k-suffix for counts, grouped into 1-3 lines (CBC/diff line, chem line, inflammatory line). End the Lab section with one interpretation line starting with '\u2192', e.g. '\u2192 no leukocytosis, no sig. inflammatory marker elevation'. Include only labs present in the source.",
  "Image: one line per study in 'CXR: Lt CP angle blunting' style; keep sizes and locations when present in the source.",
  "Imp: the working impression, e.g. 'Suspect multidermatomal/disseminated HZ / VZV infx'.",
  "Abbreviate aggressively: infx, HZ, bil, Lt/Rt, RLE/RUE, HA, N (normal), OSH, abd, sig., CP angle, s/p, c/f, r/o, f/u.",
